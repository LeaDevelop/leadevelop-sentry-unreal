...
void USentrySubsystem::Initialize()
{
	...
	// @LeaDevelop customization
	SENTRY_INIT_CRASH_TESTER(this);
...
void USentrySubsystem::AddDefaultContext()
{
	...
	// @LeaDevelop
	// Custom tags added on top of default Sentry tags
	// engine version - 5.6.1
	// changelist - 123456
	// level name - Lvl_FirstPerson
	// world context (game, PIE, editor, ...)
	
	FEngineVersion EngineVersion = FEngineVersion::Current();
	
	TMap<FString, FSentryVariant> DefaultContext;
	DefaultContext.Add(TEXT("Engine version"), FEngineVersion::Current().ToString(EVersionComponent::Patch));
	DefaultContext.Add(TEXT("Changelist"), FString::FromInt(EngineVersion.GetChangelist()));
...
void USentrySubsystem::PromoteTags()
{
	...
	// @LeaDevelop customization
	if (Settings->TagsPromotion.bPromoteChangelist)
	{
		SubsystemNativeImpl->SetTag(TEXT("Changelist"), FString::FromInt(FEngineVersion::Current().GetChangelist()));
	}
	
	// @LeaDevelop customization
	if (Settings->TagsPromotion.bPromoteEngineVersion)
	{
		SubsystemNativeImpl->SetTag(TEXT("EngineVersion"), FEngineVersion::Current().ToString(EVersionComponent::Patch));
	}
...
void USentrySubsystem::ConfigureBreadcrumbs()
{
	...
	{
		// @LeaDevelop customization
		PreLoadMapDelegate = FCoreUObjectDelegates::PreLoadMap.AddWeakLambda(this, [this](const FString& IncomingMapName)
		{
			const USentrySettings* Settings = FSentryModule::Get().GetSettings();

			// @LeaDevelop customization
			if (Settings->TagsPromotion.bPromoteLevelName)
			{
				FString FormattedMapName = FormatLevelName(IncomingMapName);
				SetTag("LevelName", FormattedMapName);
				SetTag("WorldContext", TEXT("Loading"));
			}
			AddBreadcrumbWithParams(TEXT("PreLoadMap"), TEXT("Unreal"), TEXT("Default"), { { TEXT("Map"), IncomingMapName } }, ESentryLevel::Info);
		});
	}
	...
	if (Settings->AutomaticBreadcrumbs.bOnMapLoaded)
	{
		PostLoadMapDelegate = FCoreUObjectDelegates::PostLoadMapWithWorld.AddWeakLambda(this, [this](UWorld* World)
		{
			...
			if (World)
			{
				FString LoadedMapName = FormatLevelName(World->GetMapName());
				...
				{
					WorldTypeStr = TEXT("Game");
				}
				else if (World->WorldType == EWorldType::Editor)
				{
					WorldTypeStr = TEXT("Editor");
				}
				
				if (Settings->TagsPromotion.bPromoteLevelName)
				{
					SetTag("LevelName", LoadedMapName);
					SetTag("WorldContext", WorldTypeStr);
				}
				AddBreadcrumbWithParams(TEXT("PostLoadMapWithWorld"), TEXT("Unreal"), TEXT("Default"), { { TEXT("Map"), LoadedMapName } }, ESentryLevel::Info);
			}
			else
			{
				if (Settings->TagsPromotion.bPromoteLevelName)
				{
					SetTag("LevelName", TEXT("LoadFailed"));
					SetTag("WorldContext", TEXT("Failed"));
				}
	...
	if (Settings->TagsPromotion.bPromoteLevelName)
	{
		WorldInitializedDelegate = FWorldDelegates::OnPostWorldInitialization.AddWeakLambda(this, [this](UWorld* World, const UWorld::InitializationValues IVS)
		{
			if (World)
			{
				FString CurrentLevel = FormatLevelName(World->GetMapName());
				FString WorldTypeStr = TEXT("Unknown");
				...
				// Only update tags if the level has changed
				if (LastLevelName != CurrentLevel || LastWorldContext != WorldTypeStr)
				{
					SetTag("LevelName", CurrentLevel.IsEmpty() ? TEXT("Unknown") : CurrentLevel);
					SetTag("WorldContext", WorldTypeStr);

					LastLevelName = CurrentLevel;
					LastWorldContext = WorldTypeStr;
				}
			}
		});
	}
...
// @LeaDevelop customization
FString USentrySubsystem::FormatLevelName(const FString& RawMapName) const
{
	FString FormattedName = RawMapName;
    
	// Remove PIE prefix (UEDPIE_0_, UEDPIE_1_, etc.)
	if (FormattedName.StartsWith(TEXT("UEDPIE_")))
	{
		int32 UnderscoreIndex = FormattedName.Find(TEXT("_"), ESearchCase::CaseSensitive, ESearchDir::FromStart, 7);
		if (UnderscoreIndex != INDEX_NONE)
		{
			FormattedName = FormattedName.Mid(UnderscoreIndex + 1);
		}
	}
    
	// Remove common prefixes
	FormattedName.RemoveFromStart(TEXT("/Game/"));
	FormattedName.RemoveFromStart(TEXT("/Engine/"));
    
	// Remove file extensions
	FormattedName.RemoveFromEnd(TEXT("_Built"));
    
	// Extract just the map name from path
	int32 LastSlashIndex = FormattedName.Find(TEXT("/"), ESearchCase::CaseSensitive, ESearchDir::FromEnd);
	if (LastSlashIndex != INDEX_NONE)
	{
		FormattedName = FormattedName.Mid(LastSlashIndex + 1);
	}
    
	// Handle common Unreal map names
	if (FormattedName == TEXT("Entry"))
	{
		return TEXT("Entry Level");
	}
